...
class Formatter:
    ...
    ctrl_1line = re.compile(
        r"(^|\s*)(if|while|for|try)(\W\s*\S.*\W)((end|endif|endwhile|endfor);?)(\s+\S.*|\s*$)"
    )
    fcnstart = re.compile(r"(^|\s*)(function|classdef)\s*(\W\s*\S.*|\s*$)")
    ctrlstart = re.compile(
        r"(^|\s*)(if|while|for|parfor|try|methods|properties|events|arguments|enumeration)\s*(\W\s*\S.*|\s*$)"
    )
    ctrl_ignore = re.compile(r"(^|\s*)(import|clear|clearvars)(.*$)")
    ctrlstart_2 = re.compile(r"(^|\s*)(switch)\s*(\W\s*\S.*|\s*$)")
    ctrlcont = re.compile(
        r"(^|\s*)(elseif|else|case|otherwise|catch)\s*(\W\s*\S.*|\s*$)"
    )
    ctrlend = re.compile(
        r"(^|\s*)((end|endfunction|endif|endwhile|endfor|endswitch);?)(\s+\S.*|\s*$)"
    )
    ...
    def cleanLineFromStringsAndComments(self, line):
        ...
        if split:
            return (
                self.cleanLineFromStringsAndComments(split[0])
                + " "
    ...
    def formatLine(self, line):
        ...
        if (
            re.match(self.block_close, strippedline)
            or self.islinecomment
            or self.isblockcomment
        ):
            self.continueline = 0
        else:
            self.continueline = self.longline
        if (
            re.match(self.ellipsis, strippedline)
            and not self.islinecomment
            and not self.isblockcomment
        ):
            self.longline = 1
        else:
            self.longline = 0
        ...
        if m:
            return (
                0,
                self.indent(-1) + m.group(2) + " " + self.format(m.group(3)).strip(),
            )
        ...
        if m:
            ...
            return (
                -step,
                self.indent(-step) + m.group(2) + " " + self.format(m.group(4)).strip(),
            )
    ...
    def format_source(self, source, start=1, end=None):
        """Format MATLAB source string. Returns formatted string (no file I/O)."""
        rlines = source.splitlines()
        rlines = rlines[start - 1 : end] if end is not None else rlines[start - 1 :]
        wlines = []
    ...
    if len(sys.argv) < 2:
        ...
        for arg in sys.argv[2:]:
            ...
            if any(char.isdigit() for char in value):
                ...
                value = False
            settings_from_args[key.strip().replace("--", "")] = value

        indent = settings_from_args.get("indentWidth", options["indentWidth"])
        start = settings_from_args.get("startLine", options["startLine"])
        end = settings_from_args.get("endLine", options["endLine"])
        sep = settings_from_args.get("separateBlocks", options["separateBlocks"])
        mode = settings_from_args.get(
            "indentMode", indentModes[str(options["indentMode"])]
        )
