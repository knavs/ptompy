...
| `ptompy.py` | Core logic: read `.p` → descramble → uncompress (stdlib zlib) → decode bytecode → write `.m`. API: `init()`, `parse(path)`. `parse()` returns `(code, msg)`; catches `KeyboardInterrupt` and `Exception`. |
| `build_nuitka.py` | Nuitka build script. Produces standalone `build/` (or `main.dist/`) with `ptompy.exe`. |
...
- **tools/** – Utility scripts, e.g. `find_scramble_table.py` (run as `python tools/find_scramble_table.py ...`).
- **icons/** – App icons (window, buttons, panel logo). Used by `main.py` and Nuitka build. Favicon can live here as `icons/favicon.ico` or in project root as `favicon.ico`; Nuitka prefers `icons/favicon.ico`.
